...
{
    assert( mb != NULL );
    return cnt_memblock_impl_size( MBIMPL_PTR( mb ) );
}

size_t cnt_memblock_capacity( const CntMemblock *mb )
{
    assert( mb != NULL );
    return cnt_memblock_impl_capacity( MBIMPL_PTR( mb ) );
}

size_t cnt_memblock_available( const CntMemblock *mb )
{
    assert( mb != NULL );
    return cnt_memblock_impl_available( MBIMPL_PTR( mb ) );
}

void  *cnt_memblock_begin( CntMemblock *mb )
{
    assert( mb != NULL );
    return cnt_memblock_impl_begin( MBIMPL_PTR( mb ) );
}

void  *cnt_memblock_end( CntMemblock *mb )
{
    assert( mb != NULL );
    return cnt_memblock_impl_end( MBIMPL_PTR( mb ) );
}

void  *cnt_memblock_at( CntMemblock *mb, size_t position )
{
    assert( mb != NULL );
    return cnt_memblock_impl_at( MBIMPL_PTR( mb ), position );
}

const void  *cnt_memblock_const_begin(const CntMemblock *mb)
{
    assert( mb != NULL );
    return cnt_memblock_impl_const_begin( MBIMPL_PTR( mb ) );
}

const void  *cnt_memblock_const_end(const CntMemblock *mb)
{
    assert( mb != NULL );
    return cnt_memblock_impl_const_end( MBIMPL_PTR( mb ) );
}

const void  *cnt_memblock_const_at(const CntMemblock *mb, size_t position)
{
    assert( mb != NULL );
    return cnt_memblock_impl_const_at( MBIMPL_PTR( mb ), position);
}

int cnt_memblock_reserve ( CntMemblock *mb, size_t new_size )
{
    assert( mb != NULL );
    return cnt_memblock_impl_reserve ( MBIMPL_PTR( mb ), new_size );
}

int cnt_memblock_resize(CntMemblock *mb, size_t new_size)
{
    assert( mb != NULL );
    return cnt_memblock_impl_resize( MBIMPL_PTR( mb ), new_size);
}
...
{
    assert( mb != NULL );
    cnt_memblock_impl_zero( MBIMPL_PTR( mb ) );
}

void *cnt_memblock_create_back( CntMemblock *mb, size_t count )
{
    assert( mb != NULL );
    return cnt_memblock_impl_create_back( MBIMPL_PTR( mb ), count );
}
...
{
    assert( mb != NULL );
    return cnt_memblock_impl_create_insert( MBIMPL_PTR( mb ), position, count );
}

void *cnt_memblock_delete( CntMemblock *mb, size_t position, size_t count )
{
    assert( mb != NULL );
    return cnt_memblock_impl_delete( MBIMPL_PTR( mb ), position, count );
}

void cnt_memblock_reduce( CntMemblock *mb, size_t count )
{
    assert( mb != NULL );
    cnt_memblock_impl_reduce( MBIMPL_PTR( mb ), count );
}

int cnt_memblock_extend( CntMemblock *mb, size_t count )
{
    assert( mb != NULL );
    return cnt_memblock_impl_resize( MBIMPL_PTR( mb ),
             cnt_memblock_impl_size( MBIMPL_PTR( mb ) ) + count );
}

int cnt_memblock_append( CntMemblock *mb, const void *data, size_t len )
{
    assert( mb != NULL );
    return cnt_memblock_impl_append( MBIMPL_PTR( mb ), data, len );
}
